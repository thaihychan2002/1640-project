...
export default function Postdetail({ post }) {
  ...
  const checkaction = actionslog.filter(
    (action) => action.postID._id === post._id && action.action !== "Viewed"
  );
  checkaction.map((action) => (actiondata.current._id = action._id));
  checkaction.map((action) => (actiondata.current.action = action.action));
  const checkviewed = actionslog.filter(
    (action) => action.postID._id === post._id && action.action === "Viewed"
  );
  ...
  const [likeActive, setLikeActive] = React.useState(
    actiondata.current.action === "Like" ? true : false
  );
  const [dislikeActive, setDislikeActive] = React.useState(
    actiondata.current.action === "Dislike" ? true : false
  );
          ...
          <Card className={classes.card} key={post._id}>
            <CardHeader
              avatar={
                post.isAnonymous ? (
                  <img
                    src={animal.current.avatar}
                    alt={`${animal.current.name} Avatar`}
                  />
                ) : (
                  <img
                    src={post?.author?.avatar}
              ...
              <div>
                ...
                >
                  ...
                  <Typography
                    component="span"
                    color="textSecondary"
                  >{`${post.dislikeCount} dislikes`}</Typography>
                </IconButton>
              </div>
